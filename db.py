...
class MODULE_RASP_DB:
    ...
    def timetableGroupDate_exist(self, ti_group,  ti_lessonDate = '2010-01-01'):
        with self.connection:
            result = self.cursor.execute(
                "SELECT * FROM `ti_timetable` WHERE `ti_group` = ? AND `ti_lessonDate` = ? ",
                 (ti_group, ti_lessonDate)).fetchall()
            return bool(len(result))
    ...
    def delete_timetable(self, ti_group, ti_lessonDate):
        with self.connection:
            self.cursor.execute("DELETE FROM `ti_timetable` WHERE `ti_group` = ? AND ti_lessonDate = ?", (ti_group, ti_lessonDate))
